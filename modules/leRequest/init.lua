...
do
  local _class_0
  local makeRequest, itrUrlParams, _formatParams, formatParams, checkURL, checkDATA, basicAuthHeader, md5Hash, digestHashResponse, digestCreateHeaderString, digestAuthHeader, addAuthHeaders, createHeader, checkTIMEOUT, checkREDIRECT, parseArgs, parseDigestResponseHeader, useDigest
  ...
  formatParams = function(url, params)
    local urlParam
    for k, v, URL in itrUrlParams(url, params) do
      urlParam = URL
    end
    return urlParam
  end
